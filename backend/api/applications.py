...
def get_applications(db: Session = Depends(get_db)):
    ...
    return db.query(Application).options(joinedload(Application.internship)).all()

@router.post("", response_model=ApplicationResponse, status_code=status.HTTP_201_CREATED)
def create_application(app_create: ApplicationCreate, db: Session = Depends(get_db)):
    """Save an internship to the tracker or update its status if already tracked."""
    ...
    # Check if already applied/saved; if so, update status
    existing_app = db.query(Application).filter(Application.internship_id == app_create.internship_id).first()
    if existing_app:
        existing_app.status = app_create.status
        if app_create.notes is not None:
            existing_app.notes = app_create.notes
        if app_create.status in ["applied", "interview", "offer", "rejected"] and existing_app.applied_date is None:
    ...
    new_app = Application(
        ...
        applied_date=datetime.now(timezone.utc) if app_create.status in ["applied", "interview", "offer", "rejected"] else None
    )
    db.add(new_app)
    db.commit()
    db.refresh(new_app)
